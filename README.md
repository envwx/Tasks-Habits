# Tasks&Habits
## Date: 11/11/2025

### By: Naser hubaishi
### | [Naser GitHub](https://github.com/envwx)
### | [Trello](https://trello.com/b/3GzNRerW/my-trello-board)
***
### ***Description***
Tasks&Habits, an app to keep on track of your tasks and able to create habits that matches your daily life
***

####
***
## ERD & Figma
###  ![ERD](./images/ERD.png)
### | [Figma Link](https://www.figma.com/design/GodwZBFJ4qW0YaEdEcQasu/Tasks-Habits?node-id=0-1&p=f&t=1INiBVEnRvEZjRuy-0)
***
### ***Technologies used in our project***
  * python
  * sql
  * django

***

# Featuring in the project

* creating tasks
* adding habits
* users will be able to delete tasks/habits
* users will be able to edit both the tasks and habits
* allowing user to login and only show their tasks/habits
* while adding and creating habits & tasks users will be able to view them from the HomePage
* full separate pages for habits/tasks to show the full lists of each
* users will be able to complete their tasks/habits and it will be displayed
***

## ***Project Preview***

![HomePage](/images/homepage.png)
![TasksPage](/images/TasksPage.png)
![HabitsPage](/images/HabitsPage.png)
  ***

## ***Future Work***
* adding a profile page for users
* an extra feature where user can pick a random task/habit when there's more than +3
* re styling the pages and much more!
